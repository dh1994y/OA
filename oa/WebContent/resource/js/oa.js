...
function createMenu(config) {
	...
	var leftNavList = config.leftNavList;// leftNavItem leftNavItem[0] name
											// url
	...
	for (var i = 0; i < mainNavCount; i++) {
		...
		var mainNavItemDom = $('<li id="' + mid + '" leftMark="' + lid
				+ '" rightMark="' + rid + '"><a href="javascript:void(0);">'
				+ mainNavList[i] + '</a></li>');
		...
		mainNavItemDom
				.click(function() {
					// 当前添加活动样式 其余移除活动样式
					$(this).addClass("myactive").siblings().removeClass(
							"myactive");
					// 切换左侧导航栏
					var leftNavId = $(this).attr("leftMark");
					$('#' + leftNavId).addClass("li-show").remove('li-hidden')
							.siblings().removeClass("li-show").addClass(
									"li-hidden");
					// 切换时自动点击左侧第一选项卡
					var leftNavliArr = $('#' + leftNavId).children(":first")
							.children();
					...
					for (var k = 0; k < leftNavliArr.size(); k++) {
						...
						if (k == leftNavliArr.size() - 1) {
							$('#' + leftNavId).children(":first").children(
									":first").click();
						}
					}
					// 切换右侧内容区
					var rightContentId = $(this).attr("rightMark");
					$('#' + rightContentId).addClass("li-show").remove(
							'li-hidden').siblings().removeClass("li-show")
							.addClass("li-hidden");
				});
		...
		for (var j = 0; j < leftNavList[i].length; j++) {
			...
			var leftNavItemUlLiDom = $('<li id="' + lnavid
					+ '" flag="close" didMark="' + did + '" url="'
					+ leftNavList[i][j].url
					+ '"><a href="javascript:void(0);">'
					+ leftNavList[i][j].name + '</a></li>');
			...
			leftNavItemUlLiDom
					.click(function() {
						...
						if ($(this).attr("flag") == "open") {
							...
							// 定位
							// 当前为活动其余非活动样式
							$(this).addClass("leftactive").siblings()
									.removeClass("leftactive");
							$("#" + $(this).attr("drtidMark")).removeClass(
									"myli-close").addClass("myli-active")
									.siblings().removeClass("myli-active")
									.addClass("myli-close");
							$("#" + $(this).attr("drbidMark")).removeClass(
									"li-hidden").addClass("li-show").siblings()
									.removeClass("li-show").addClass(
											"li-hidden");
							return;
						}
						$(this).attr("flag", "open");
						...
						if (!$(this).hasClass("leftactive")) {
							...
							var rtLabelDom = $('<li id="'
									+ drtid
									+ '" class="myli-active" drbidMark="'
									+ drbid
									+ '" lidMark="'
									+ $(this).attr("id")
									+ '"><a>'
									+ $(this).children(":first").text()
									+ '</a><img src="/oa/resource/image/close.png"/></li>');
							$(this).attr("drtidMark", drtid).attr("drbidMark",
									drbid);
							// 添加到容器
							$("#" + $(this).attr("didMark")).children(":first")
									.children(":first").append(rtLabelDom);
							rtLabelDom.siblings().removeClass("myli-active")
									.addClass("myli-close");
							// 为右上标签添加点击事件
							rtLabelDom
									.children(":first")
							...
							// 创建右下iframe
							var rbIframeDom = $('<li id="' + drbid
									+ '" class="li-show"><iframe src="'
									+ $(this).attr("url") + '"></iframe></li>');
							// 添加到容器
							$("#" + $(this).attr("didMark")).children(":last")
									.children(":first").append(rbIframeDom);
							rbIframeDom.siblings().removeClass("li-show")
									.addClass("li-hidden");
		...
		// 创建rightContentDom
		var rightContentDom = $('<li id="'
				+ rid
				+ '"><div id="'
				+ did
				+ '" class=".content-div">'
				+ '<div id="page-middle-right-top"><ul id="page-middle-right-nav" ></ul></div><div id="page-middle-right-below"><ul id="page-middle-right-content"></ul></div>');
...
function showDT() {
	...
	window.setTimeout(showDT, 1000*60);
}